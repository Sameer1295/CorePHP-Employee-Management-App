...
if(isset(($_SESSION['u_name']))){
    ...
    if(isset($_POST['search_button'])){
        ...
        $obj2 = new Employercontr;
    
        $result = $obj2->showEmployerByName($emp_name);
    }
...
<body>
    ...
    <div class="container">
        <div class="row">
            ...
            <h2>All Employers</h2>
            <?php if(empty($result)): ?>
                <p class="alert-danger">No Employer added</p>
            <?php endif ?>
            <?php if(!empty($result)): ?>
                ...
                <?php for($i = 0;$i < count($result);$i++): ?>
                    <tr>
                    <th scope="row"><?php echo $result[$i]['employer_id']; ?></th>
                    <td><?php echo $result[$i]['employer_name']; ?></td>
                    <td><?php echo $result[$i]['employer_address']; ?></td>
                    <td><?php echo $result[$i]['employer_contact_number']; ?></td>
                    <td>
                        <a href="detail_employer.php?e_id=<?php echo $result[$i]['employer_id'] ?>" class="btn btn-success">View</a>
                        <a href="edit_employer.php?e_id=<?php echo $result[$i]['employer_id'] ?>" class="btn btn-warning">Edit</a>
                        <button onclick="deleteEmployer('<?php echo $result[$i]['employer_id'] ?>')" class="btn btn-danger">Delete</button>
                    </td>
                    </tr>
                <?php endfor ?>
                </tbody>
    ...
    <script>
    function deleteEmployer(e_id){
        if(
            confirm('are you sure, you want to delete?')
        ){
            window.location.href="delete_employer.php?e_id="+e_id
        }
    }
    </script>
